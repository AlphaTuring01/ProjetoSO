...
int main(int argc, char *argv[]) {
    ...
    for(int index=1;index<argc;index++) {
        ...
        if(posicaoDaPrimeiraVirgula == std::string::npos || posicaoDaUltimaVirgula == std::string::npos) {
        ...
        int burstDeCpu         = std::stoi(stringProcesso.substr(posicaoDaPrimeiraVirgula+1,posicaoDaUltimaVirgula - posicaoDaPrimeiraVirgula - 1));
        int quantidadeDeBursts = std::stoi(stringProcesso.substr(posicaoDaUltimaVirgula+1,stringProcesso.size() - posicaoDaUltimaVirgula - 1));

        processos.push_back(Processo(nome,burstDeCpu,quantidadeDeBursts));
